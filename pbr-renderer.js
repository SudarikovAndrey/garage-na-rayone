...
export function skyMaterial(cfg){
 return new T.ShaderMaterial({side:T.BackSide,depthWrite:false,fog:false,
  uniforms:{sky:{value:new T.Color(cfg.sky)},horizon:{value:new T.Color(cfg.fog)},sun:{value:new T.Color(cfg.sun)},
   power:{value:cfg.intensity},sunDir:{value:new T.Vector3(...(cfg.id==='factory'?[-24,7,-30]:[12,24,-30])).normalize()}},
  vertexShader:'varying vec3 v;void main(){v=position;gl_Position=projectionMatrix*modelViewMatrix*vec4(position,1.);}',
  fragmentShader:'varying vec3 v;uniform vec3 sky,horizon,sun;uniform float power;uniform vec3 sunDir;void main(){vec3 d=normalize(v);float h=smoothstep(-.08,.8,d.y);vec3 c=mix(horizon,sky,h);float spot=pow(max(dot(d,sunDir),0.),160.);c+=sun*spot*power*3.;if(d.y<-.05)c*=.16;gl_FragColor=vec4(c,1.);}'});
}
