...
import {bindEvent} from './event-ui.js?v=cd0c709';
...
import {bindCustomizer} from './customizer-ui.js?v=cd0c709';
...
import {FINISH_FILM_DURATION,finishCameraFrame} from './finish-camera.js?v=cd0c709';
...
let challengeUI=null,challengeIntro=null,challengePicker=null,garageUpgrading=false;
...
async function buildRace(index=0,look=null){raceLookCfg=look||TRACKS[index];await loadWorldTextures();
 ...
 weather=new WeatherEffects(raceScene,world.cfg,reducedMotion,world.smokeMap);raceMap=index;raceEffects.setQuality(quality.settings);weather.setQuality(quality.settings);shadowBudget(raceScene.userData.sun,quality.settings);quality.reset();
}
...
async function claimChallengeRewards(){
 let pending=[];
 try{pending=(await challengeApi.rewards()).pending||[];}catch{return;}
 const {applied,got}=applyReceipts(save,pending);
 if(!applied.length)return;
 persist();renderGarageUI();
 if(!gotSomething(got))return;
...
function frame(now){
 ...
 if(screen==='garage'){garageYaw+=garageIdle.update(dt,dragging||customizer?.isOpen||sceneTransition.busy||!!document.querySelector('dialog[open]'),reducedMotion);const moving=updateGarageCamera(1-Math.exp(-dt*12)),glow=selectionGlow?.update(dt,CARS[save.selected],customizer?.focusSlot,inspectionAnchor,reducedMotion),blink=garageIndicators?.update(dt),neonGlow=garageCar?.userData.neon?.update(dt),lightShift=garageBlackout?.update(dt);if(blink||neonGlow)garageLayout?.syncMirror();/* Теневая карта — только когда сцена под тенью изменилась: машина, стены и декор, что перестали или начали заслонять. Камера орбитой солнце не двигает, а пересчёт 1024² на каждом кадре вращения был самой дорогой частью кадра. */if(garageDirty||garageShadowDirty||garageLayout?.takeShadowDirty()){garageScene.userData.sun.shadow.needsUpdate=true;garageShadowDirty=false;}if(garageDirty||moving||glow||blink||neonGlow||lightShift){graphics.render(garageScene,garageCamera);garageDirty=false;}}
 else{
  ...
  tickNeon(dt);smoke(dt);if(introActive){introElapsed+=dt;if(!reducedMotion&&introStep===2)bossIntroCamera(introElapsed,introTotal);if(introElapsed>=introTotal&&!advanceIntro())finishIntro();}else updateRaceCamera(dt);weather?.update(dt,player,rival,playerModel,rivalModel);updateDashboard();graphics.render(raceScene,raceCamera);
 }
 updateAudio();
}
