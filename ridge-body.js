...
export function beginRidgeCrash(car){
 ...
 const b={p:[point.x,cg-.02,point.z],v:[s*forward+c*sideSpeed,0,-c*forward+s*sideSpeed],q,w:[0,-(car.ridgeYawRate||0),0],cg,contacts,obstacles:car.ridgeObstacles||[],
  inertia:[(fit.height**2+fit.length**2)/12,(fit.width**2+fit.length**2)/12,(fit.width**2+fit.height**2)/12].map(v=>v*1.1),hint:car.distance,elapsed:0,accumulator:0,firstImpact:null,impacts:0,impact:0,grounded:0,airTime:0,settleTime:0,resting:false,rolled:false,maxAngularSpeed:0,wheelSpeed:forward};
...
function step(b,dt){
 ...
 for(const contact of b.contacts){
  ...
  function register(normal,penetration){const arm=add(r,scale(normal,-contact.radius)),vn=dot(add(b.v,cross(b.w,arm)),normal);active.push({contact,r:arm,n:normal,depth:penetration,bounce:vn< -4?-vn*.035:0,normal:0,push:0,tangent:[0,0,0],surface});if(vn< -2){b.impact=Math.max(b.impact,-vn);b.firstImpact??=b.elapsed;b.impacts++;}}
...
export function ridgeCrashPresentationDone(car,reducedMotion=false){
 ...
 return b.elapsed>=(reducedMotion?1.8:3)||(b.elapsed>=1.4&&((b.firstImpact!==null&&b.elapsed-b.firstImpact>=.75)||b.resting));
}
