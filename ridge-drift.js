...
// Занос стоит скорости. Газ в заносе: стоковая Лада на 90 км/ч боком не может продолжать разгоняться —
// без потолка тяга перекрывала скраб на любом угле до 33°, и удержанный бегунок ехал полубоком без потерь.
// HOLD_EXPIRY — сколько секунд занос живёт на прямой без боковой нагрузки, прежде чем задняя ось снова зацепится.
export const SLIDE_THROTTLE=.7,HOLD_EXPIRY=1.8;
...
export function tickDrift(car,dt){
 ...
 // Заносу на прямой не на чем держаться: без кривизны задняя ось за HOLD_EXPIRY снова цепляется, и занос
 // гаснет; в дуге держится. Перекладка — новый перенос веса, её таймер с нуля. Погасший занос удержанием
 // не заводится, пока бегунок не отпустят ниже порога входа: только рывком. Прямые всех трасс — ровно κ=0,
 // самый пологий поворот Гребня κ≈.0026; порог между ними.
 const loaded=smooth(.0008,.0018,Math.abs(c.curvature));
 d.lockArmed??=true;if(input<DRIFT_ENTER)d.lockArmed=true;
 const transfer=d.mode==='slide'&&d.steer*(d.slideSteer??d.steer)<0;if(d.mode==='slide')d.slideSteer=d.steer;
 d.straightTime=d.mode!=='slide'||loaded>=.5||flick||transfer?0:(d.straightTime||0)+dt;
 if(d.mode==='slide'&&d.straightTime>HOLD_EXPIRY){
  d.mode='recover';d.recoveryTime=0;d.lockArmed=false;d.straightTime=0;
 }else if(car.speed>=12&&(flick||d.strongTime>.16&&d.lockArmed)){
 ...
 const driveProfile=updateDriftAxles(car,dt,driftCourse().terrain,undefined,c.curvature);if(sliding)d.throttle=Math.min(d.throttle,SLIDE_THROTTLE);
 ...
 // На прямой задняя ось цепляется постепенно: к угасанию сцепление уже почти вернулось, выход короткий.
 const gripTarget=sliding?(.24+d.rearGrip*.23)+smooth(0,HOLD_EXPIRY,d.straightTime||0)*.6:recovering?1-.40*smooth(.08,.75,slipLoad):1;
 ...
 if(!physical){
  ...
  if(sliding){const target=driftSteerAngle(d.steer)*driveProfile.angle;const pull=driveProfile.front*d.frontGrip*(1-d.rearBrake)*driveProfile.align;torque=(target-d.angle)*(42+16*(1-driveProfile.front))-d.angularVelocity*(12+2*driveProfile.front)-Math.sin(d.angle)*pull;}
   ...
   torque=-Math.sin(d.angle)*(2.2+d.grip*3.8+driveProfile.front*d.frontGrip*driveProfile.align)-d.angularVelocity*(1.7+d.grip*2+driveProfile.front*1.2)+(d.steer*d.angle>0?0:d.steer*10*smooth(.06,.5,Math.abs(d.angle)));/* руль в сторону заноса при возврате сцепления — не контрруль: он поворачивает через боковую скорость, а не крутит кузов; иначе удержанный бегунок закручивал машину до 160° */
 ...
 const scrub=slip*(2.2+car.speed*.45)*driveProfile.scrub+(d.rearBrake||0)*(1.1+car.speed*.035),drive=accel*(1-(car.speed/top)**2)*(1-slip*.65)*d.throttle;
