...
import {ACCEPT_LABEL} from './challenge-rules.js';
...
<div class="challenge-intro-actions"><button class="primary" data-accept></button><button class="text-button" data-later>ПОТОМ</button></div>`;
 ...
 function show(card){
  ...
  el.querySelector('[data-time]').textContent=card.timeA>0?'Его время: '+card.timeA.toFixed(2)+' с':'Он ещё не проехал';
  el.querySelector('[data-accept]').textContent=ACCEPT_LABEL;
