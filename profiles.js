...
export function profileMarkup(profiles,save,cloudList=[],{required=false,dev=false,photo=null,name=null}={}){
 ...
 if(required){
  // Первый запуск: имя обязательно — без него прогресс некуда сохранить и нечем подписать плейтест.
  const pick=list?`<p class="crate-note">Уже играл? Выбери себя:</p>${list}`:'';
  return `<div class="event-season"><span data-dev-tap>НОВЫЙ ИГРОК</span><b>ГАРАЖ НА РАЙОНЕ</b><small>${url?'ПРОГРЕСС СОХРАНИТСЯ И ОТКРОЕТСЯ НА ЛЮБОМ УСТРОЙСТВЕ':'ПРОГРЕСС СОХРАНИТСЯ В ЭТОМ БРАУЗЕРЕ'}</small></div>
  <section class="event-profile profiles profiles-required">${portrait}<h3>Как тебя звать на районе?</h3>
  <span class="event-league">БЕЗ ИМЕНИ ПРОГРЕСС НЕ СОХРАНИТЬ</span>
  <form class="profile-new" data-profile-create><label for="profile-name">ПОЗЫВНОЙ</label><input id="profile-name" name="name" maxlength="24" placeholder="например, Андрей" autocomplete="off" spellcheck="false" required autofocus><button class="primary" type="submit">В ГАРАЖ</button></form>
  ${pick}
  <p class="event-note">${url?'Позывной заменяет пароль: набери его на другом телефоне — и продолжишь с того же места.':'Приёмник выключен на этом устройстве: позывной живёт только здесь.'}</p></section>`;
 }
