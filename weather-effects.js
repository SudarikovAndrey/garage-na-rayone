...
export class WeatherEffects{
 constructor(scene,cfg,reduced=false,smokeMap=null){
  Object.assign(this,{scene,cfg,reduced,time:0,travel:0,flash:0,nextFlash:5,splashes:[],glows:[],lamps:[],beams:[],resources:[],density:.7});if(!cfg.wet)return;
  ...
  const count=Math.round((cfg.night?1500:700)*(1+this.storm*1.1));this.drops=Array.from({length:count},()=>({x:(Math.random()-.5)*34,y:Math.random()*18,z:Math.random()*62-36}));
...
#include <colorspace_fragment>
 ...
 update(dt,player,rival,playerModel,rivalModel){
  ...
  const fall=(this.cfg.night?20:14)*(1+this.storm*.45),streak=(this.cfg.night?.72:.38)*(1+this.storm*.9),wind=this.wind;for(let i=0;i<active;i++){const d=this.drops[i];d.y-=dt*fall;d.x+=dt*1.7;d.z+=delta*.55+dt*wind;if(d.y<0){d.y=16+Math.random()*2;d.x=(Math.random()-.5)*34;d.z-=wind*.8;}if(d.z>26)d.z-=62;const j=i*6;positions[j]=d.x;positions[j+1]=d.y;positions[j+2]=d.z;positions[j+3]=d.x-.055;positions[j+4]=d.y+streak;positions[j+5]=d.z-speed*.012-streak*wind/fall;}this.rain.geometry.attributes.position.needsUpdate=true;
