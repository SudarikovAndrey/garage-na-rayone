...
const paints=['cherry','ivory','black','blue','green','silver','orange'];
...
export function normalizeDuelBuild(raw={}){const car=integer(raw.car,0,CARS.length-1),levels=[0,1,2].map(i=>clamp(Number(raw.levels?.[i])||0,0,50)),equipment={};for(const slot of slots){const id=raw.equipment?.[slot];if(new RegExp('^'+slot+'-[0-3]$').test(id))equipment[slot]=id;}const paint=paints.includes(raw.paint)?raw.paint:CARS[car].color,decal=decalById(raw.decal)?.id||null,power=vehicleStats(CARS[car].id,levels).rating;
 ...
 return {car,levels,power,classId:duelClassFor(power).id,equipment,paint,decal,kit};}
