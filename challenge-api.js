...
export function bindChallengeApi({fetchImpl=(...a)=>fetch(...a),headers=()=>({}),storage=globalThis.localStorage}={}){
 ...
 return {
  // Мои стрелки: кто ждёт ответа, кого жду я, чем кончилось.
  list:()=>call('/challenges'),
  // Создать вызов. rivalId и parentId заполняются только у реванша и адресного вызова.
  create:body=>call('/challenge',body||{}),
  card:id=>call('/challenge/'+id),
  accept:id=>call('/challenge/'+id+'/accept',{}),
  // Выход на старт отдельно от приёма: между ними человек идёт прокачиваться, и сборку
  // сервер замораживает именно здесь.
