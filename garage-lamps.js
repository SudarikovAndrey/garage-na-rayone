...
import {lensCorners} from './car-lamps.js';
...
export function bindIndicators(root,{reduced=false,lights:withLights=true}={}){
 ...
 const y=(box.min.y+box.max.y)/2,lights=[],corners=lensCorners(T,root,meshes);
 if(!reduced&&withLights)for(const sx of [-1,1])for(const sz of [-1,1]){
  const light=new T.PointLight(0xffa02c,0,BLINK.reach,2);
  const corner=corners?.[(sx>0?1:0)+(sz>0?2:0)];if(corner)light.position.copy(corner);else light.position.set(sx>0?box.max.x-.1:box.min.x+.1,y,sz>0?box.max.z+.14:box.min.z-.14);
