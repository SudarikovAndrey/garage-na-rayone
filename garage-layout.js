...
export const FLOOR_OPACITY={concrete:.98,cream:.92};
...
export function arrangeGarage(scene,room,level,options={}){
 ...
 function registerOccluder(object){if(!object)return;for(let p=object;p;p=p.parent)if(!p.visible)return;object.updateWorldMatrix(true,true);const box=new T.Box3().setFromObject(object);if(box.isEmpty())return;const size=box.getSize(new T.Vector3());if(size.y<.5)return;const mats=new Set();object.traverse(m=>{if(m.isMesh){if(Array.isArray(m.material))return;mats.add(m.material);}});
