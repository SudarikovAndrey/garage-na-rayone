...
export function arrangeGarage(scene,room,level,options={}){
 ...
 function setCar(next){
  ...
  car.traverse(o=>{if(!o.isMesh||!o.visible)return;const m=Array.isArray(o.material)?o.material[0]:o.material;if(!m||m.isShaderMaterial||m.isRawShaderMaterial)return;const clone=m.clone();clone.transparent=false;clone.opacity=1;clone.depthTest=false;clone.depthWrite=false;if(clone.emissive&&clone.color){clone.emissive.copy(clone.color).multiplyScalar(.28).add(clone.emissive);clone.emissiveIntensity=Math.max(1,clone.emissiveIntensity||1);}/* the mirrored body is lit from below, so lift it a little */clone.onBeforeCompile=()=>{};clone.customProgramCacheKey=()=>'garage-mirror-'+m.type;/* no box-projected probe on the mirror; key per material type so programs are not shared across types */const mesh=new T.Mesh(o.geometry,clone);mesh.userData.source=o;mesh.matrixAutoUpdate=false;mesh.matrix.copy(o.matrixWorld);mesh.matrixWorldNeedsUpdate=true;mesh.renderOrder=-1000;mesh.castShadow=false;mesh.receiveShadow=false;mesh.frustumCulled=false;mirror.add(mesh);});
 ...
 function apply(oc){for(const m of oc.materials){m.opacity=oc.opacity;m.depthWrite=oc.opacity>.5;}if(oc.object.castShadow!==undefined)oc.object.traverse(o=>{if(o.isMesh)o.castShadow=oc.opacity>.6;});}
 return {lift,liftGroup,occluders,setCar,update,dispose(){for(const g of owned)g.dispose();if(mirror){scene.remove(mirror);mirror.traverse(o=>{if(o.isMesh)o.material.dispose();});}scene.remove(liftGroup);}};
}
