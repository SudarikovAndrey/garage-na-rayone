...
export function suggestOnce(toast,shell=null){
 if(standalone()||!toast)return false;
 // В Телеграме подсказка про установку приложения не к месту: там игра и так в клиенте, а полный
 // экран переключается тем же «?». Если он уже развёрнут — подсказывать нечего.
 const external=!!shell?.available?.();
 if(external&&shell.active())return false;
 try{if(localStorage.getItem(SUGGESTED))return false;localStorage.setItem(SUGGESTED,'1');}catch{return false;}
 toast(external?'Во весь экран и обратно — кнопка в шапке, справа от звука'
  :supported()?'Играть удобнее во весь экран: кнопка «Во весь экран» в меню «?»':homeScreenHint());
 return true;
}

// shell — чужой полный экран (Телеграм): {available,active,toggle,watch}. Пока он доступен, кнопка
// управляет им, а не браузерным: внутри мини-аппа полный экран принадлежит клиенту Телеграма.
export function bindShell(button,toast,shell=null){
 if(!button)return null;
 const external=()=>!!shell?.available?.();
 const sync=()=>{
  button.textContent=external()?(shell.active()?'ВЫЙТИ ИЗ ПОЛНОГО ЭКРАНА':'ВО ВЕСЬ ЭКРАН'):shellLabel();
  button.disabled=external()?false:standalone();
 };
 button.onclick=async()=>{
  if(external()){shell.toggle();setTimeout(sync,200);return;}
