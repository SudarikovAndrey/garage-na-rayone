...
export const WAVE = {duration: 1150, swapAt: .42, radius: 15, reduced: 420};
...
export function garageUpgradeLines(prev, next) {
 ...
 if (next.discount > (prev?.discount ?? 0)) lines.push(`Тюнинг дешевле на ${Math.round(next.discount * 100)}%`);
 return lines;
}
...
export function bindGarageUpgrade({THREE, getScene, markDirty, punch, sfx, haptics, reducedMotion = false} = {}) {
 ...
 function setPhase(t) {
  ...
  ringMat.opacity = fade * .85;
  domeMat.opacity = fade * .22;
 }
