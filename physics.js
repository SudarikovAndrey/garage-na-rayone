...
export function createCar(levels=[0,0,0],variant=0,carId='samara',raceDistance=402,kit={}){
 ...
 return {levels:[...levels],variant,carId:handling.id,handling,gears,maxGear:gears.length,redline:handling.rpm,raceDistance,gear:1,rpm:1000,speed:0,distance:0,travelDistance:0,time:0,finished:false,finishTime:0,shiftPause:0,launchBoost:0,perfect:0,good:0,missed:0,bulls:0,lastBull:false,rpmRate:0,bullMs:BULLSEYE_MS,combo:0,boost:0,heat:0,window:1,surface:1,rough:0,ride:handling.ride??.5,nitro:clamp(kit.nitro||0,0,3),nitroLeft:nitroTank(clamp(kit.nitro||0,0,3)),nitroActive:false,nitroFlame:0,nitroFired:0};
}
...
// «В ТОЧКУ» — тир внутри зелёной зоны. Меряется во времени хода стрелки, а не в долях зоны:
// проход зоны на первой передаче 80 мс, на последней 1205 — доля дала бы лотерею на одной
// передаче и подарок на другой. Потолок .55 зоны держит яблочко там, где зона сама короче W.
export const BULLSEYE_MS=220;
export function bullseyeHalf(car){const [lo,hi]=shiftZone(car);return Math.min((car.rpmRate||0)*(car.bullMs??BULLSEYE_MS)/2000,(hi-lo)*.275);}
...
export function shift(car,assisted=false){
 ...
 const bull=quality==='perfect'&&Math.abs(car.rpm-p.shift)<=bullseyeHalf(car);car.lastBull=bull;if(bull)car.bulls++;
 (car.shiftLog??=[]).push({g:car.gear,rpm:Math.round(car.rpm),dev:Math.round(car.rpm-p.shift),b:bull?1:0,ms:car.rpmRate>0?Math.round((car.rpm-p.shift)/car.rpmRate*1000):null,q:quality,t:+(car.time||0).toFixed(2),v:Math.round(car.speed*3.6),a:assisted?1:0});/* per-shift quality for telemetry */
...
export function tickCar(car,dt){
 ...
 const beforeRpm=car.rpm;car.speed=car.air?car.speed:clamp(car.speed+accel*dt,0,maxSpeed);car.rpm=rpmAtSpeed(car);car.rpmRate=dt>0?Math.max(0,(car.rpm-beforeRpm)/dt):car.rpmRate;if(car.crashed)return;car.distance+=car.speed*dt;car.travelDistance=car.distance;
