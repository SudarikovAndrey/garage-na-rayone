...
export class WetRoad{
 ...
 update(dt,cars,models){
  ...
  for(let i=0;i<2;i++)for(let side=0;side<2;side++){
   ...
   const a=vehicleAnchors(model),strength=(this.cfg.night?1:.35)*(car.finished?1.6:1);
 ...
 attach(material,puddle=false){
   ...
   shader.fragmentShader=shader.fragmentShader.replace('#include <opaque_fragment>',`
    ...
    for(int i=0;i<4;i++){
     ...
     if(lamp.w>0. && dot(d,d)<49.){
      // Мокрый асфальт даёт только зеркальную дорожку от фонаря к камере. Свет, который фонарь кладёт на землю
      // сразу за бампером, рисует световой ковёр (ground-light.js) — он есть и на сухой трассе.
      ...
      // Дрожь дорожки — низкой частоты (меньше периода на метр). На 29 рад/м она на дальнем плане попадала
      // меньше чем в пиксель, и ровная полоса разваливалась в красный зигзаг (Андрей, 17 сентября).
      float ripple=sin(wetWorld.z*4.5+wetWorld.x*2.2+wetTime*1.1)*.018;
      r.x+=ripple;
      // Узкая и длинная: на экране это полоса, а не пятно. Ёлочка яркости отсюда убрана — на дальнем плане она
      // попадала меньше чем в пиксель и складывалась в муар из красных полос (Андрей, 17 сентября).
      float streak=exp(-dot(r*vec2(${puddle?'6.2,.78':'5.6,.85'}),r*vec2(${puddle?'6.2,.78':'5.6,.85'})));
      redRoad+=vec3(1.,.016,.004)*lamp.w*streak*${puddle?'.70':'.30'};
     }
    }
  ...
  material.customProgramCacheKey=()=>key+'-wet-tail-v4-'+Number(puddle);return material;
 }
