...
import {bodyWidthAt,SKIRT_STYLES,FLARE_STYLES,flareExtension} from './sculpted-body-kit.js';
...
export function buildBumperKit({car,fit,rarity,partId,kit,add,paint,dark,light=dark,drop=0,style=null,fenderRarity=null,fenderStyle=null}){rarity=kitTier(rarity);fenderRarity=kitTier(fenderRarity);
 ...
 const length=fit.length||4.05,width=fit.width,y=.50;
 // Верх штатного бампера по концам — body.bumperTop [перед, зад] в паспорте: обвесный закрывает всё, что закрывал штатный.
 // У 2112 штатный задний доходит до фонарей — на стандартной высоте под ними оставалась дыра (Андрей, 23 сентября).
 // Поле есть только у машин, где штатный бампер выше обвесного; у остальных высота прежняя.
 const stockTop={front:fit.body?.bumperTop?.[0],rear:fit.body?.bumperTop?.[1]};
 // Фары и фонари бампер не перекрывает (Андрей, 23 сентября): верх корпуса — на 1 см ниже низа их стёкол на своём конце.
 // Стёкла ниже 0.45 м — туманки в самом штатном бампере, они уходят вместе с ним.
 const lampBottom={};{car.updateWorldMatrix(true,true);const inv=car.matrixWorld.clone().invert(),v=new T.Vector3(),m=new T.Matrix4();
  car.traverse(o=>{if(!o.isMesh||!o.visible&&o.parent?.name!=='Body')return;const n=o.material?.name||'';const head=/headlight glass/i.test(n),lens=/^(Red|Amber|Clear) lens/.test(n);if(!head&&!lens)return;
   for(let p=o;p;p=p.parent)if(/^Wheel_|^Kit_|^StockBumpers/.test(p.name))return;
   m.multiplyMatrices(inv,o.matrixWorld);const a=o.geometry.attributes.position;for(let i=0;i<a.count;i++){v.fromBufferAttribute(a,i).applyMatrix4(m);if(v.y<.45)continue;
    if(v.x<-length/2+.5&&(head||/^Amber/.test(n)))lampBottom.front=Math.min(lampBottom.front??9,v.y);else if(v.x>length/2-.5&&lens)lampBottom.rear=Math.min(lampBottom.rear??9,v.y);}});}
 for(const front of [true,false]){
  ...
  // Короткий свес (нос Нивы, Копейки): губа шире кузова вылезает за колесо — там она не растёт с редкостью.
  const overhang=length/2-Math.abs((front?-1:1)*fit.wheelbase/2+(fit.axleOffset||0))-fit.radius,grow=overhang>.35?1:0;
  const shellTop=(style==='bunny'?.07:.09)+kitTier(rarity)*(style==='bunny'?.006:.008);
  end.position.set(front?-length/2+.045:length/2-.045,Math.min(Math.max(y,(stockTop[front?'front':'rear']??0)-shellTop+.01),(lampBottom[front?'front':'rear']??9)-.01-shellTop),0);end.rotation.y=front?Math.PI:0;
  ...
  if(endStyle==='ring')add(end,'Curved lower lip',sweptRail(width*(1+grow*rarity*.01),.06+rarity*.012,.012,sweep*.3,16),rarity===3?dark:paint,[.012+rarity*.004,lipY,0]);
  else if(endStyle==='rally')add(end,'Curved lower lip',sweptRail(width*.98,.06+rarity*.01,.02,sweep*.5,16),dark,[.014+rarity*.004,lipY,0]);
  else if(style!=='bunny')add(end,'Curved lower lip',sweptRail(width*(1+grow*rarity*.012),.07+rarity*.018,.012,sweep*.5,16),rarity?dark:paint,[.014+rarity*.006,lipY,0]);
  if(!style){
  ...
  // Верх возврата — по верху корпуса бампера: при возврате до пола +7 см выше него между бампером и колесом
  // оставался голый борт, и бампер «не доходил до колеса» (Андрей, 23 сентября).
  const floor=fit.body?.floor||.245,bottom=kitClearance(fit,rarity)+drop,top=Math.max(floor+.07,end.position.y+shellHalf*.8),axle=(front?-1:1)*fit.wheelbase/2+(fit.axleOffset||0),inner=fit.radius+.035+drop;
  ...
  const xStart=front?-length/2+.10:length/2-.10,extension=(SKIRT_STYLES[style]||SKIRT_STYLES[null]).ext[rarity]*.8;
  ...
  for(const side of [-1,1]){
   const geo=strip(20,profile.length,(u,j)=>{
    ...
    // Ниже верхней кромки боковина стоит в плоскости боковой стенки корпуса бампера (полуширина width·.99/2):
    // бампер читается одним куском до самой арки, а не обрывается углом с голым бортом за ним.
    const taper=1-.65*smooth((u-.78)/.22),skin=bodyWidthAt(fit,x,Math.max(y,floor+.085))+profile[j][1]*taper;
    // К арке стенка сходит на крыло, а при накладке арки — на её внешнюю кромку: у крыла уже бампера (Нива, 2112)
    // возврат стоял стенкой на 2–3 см и обрывался срезом перед аркой («кривая стыковка», Андрей, 23 сентября).
    const wall=j>=2?Math.max(skin,width*.99/2-.004):skin,flush=fenderRarity===null?skin:Math.max(skin,bodyWidthAt(fit,axle,fit.radius+.12)-.012+flareExtension(fenderRarity,!front,fenderStyle)*smooth((y-bottom)/Math.max(.01,top-bottom)));
    const z=side*(wall+(Math.min(wall,Math.max(flush,skin))-wall)*smooth((u-.45)/.55));/* на 4 мм за стенкой корпуса: без ряби на стыке */
    return front?[end.position.x-x,y-end.position.y,-z]:[x-end.position.x,y-end.position.y,z];
   },front?side<0:side>0);/* обход наружу: раньше он был вывернут на обоих концах, и возврат вдоль борта не рисовался вовсе */
