...
//
// Фара — один прожектор, а не два: на дороге лучи всё равно сходятся, а каждый источник в сцене стоит
// пересчёта всех материалов. Он создаётся сразу вместе с гаражом и просто меняет накал, поэтому число
// источников света при щелчке выключателя не меняется и шейдеры не перекомпилируются.
...
export const BLACKOUT={
 ...
 head:1.8,         // накал стёкол фар: выше порога свечения композита, но без широкого ореола
 tail:.5,          // габариты
 beam:{color:0xfff0d2,intensity:17,distance:16,angle:.46,penumbra:.5,decay:1.3},
...
export function createGarageBlackout(scene,{ambient=1,lamps:room=null,reduced=false}={}){
 ...
 function paint(){
  ...
  beam.intensity=carLamps.length?cfg.intensity*carLevel:0;
 ...
 return {
  ...
  setCar:bindCar,
  dispose(){dark=false;settle();beam.removeFromParent();beam.target.removeFromParent();fixtures.length=fill.length=glows.length=carLamps.length=0;car=null;},
 };
}
