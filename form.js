...
// Эстакада и дрифт едут против трассы: у такого соперника нет ни уровней, ни темпа — форму не трогаем.
export function applyForm(opp,form){
 if(!opp||opp.practice||opp.drift||opp.solo||!Array.isArray(opp.levels))return opp;
 // Стены формой не трогаем: они и так посчитаны против точного игрока.
 const boost=(opp.boss||opp.captain)?0:(form?.boost||0);
 if(!boost)return opp;
 return {...opp,levels:opp.levels.map(v=>v*(1+boost*2.5)),reaction:Math.max(.12,(opp.reaction??.3)-boost*7),formBoost:boost};
}
