...
export function careerMarkup(save,map,dev=false){
 ...
 // «Форма» меняет соперников на ходу, и человек должен видеть, почему они злее: серия побед
 // показана там же, где путь по кампании.
 const form=save.form?.boost>0?` · форма: ${save.form.streak} побед подряд, соперники злее на ${Math.round(save.form.boost*100)}%`:save.form?.streak>0?` · серия побед: ${save.form.streak}`:'';
 const summary=(state.complete?'все районы пройдены':`район ${cur.map+1} из 5 · серия ${cur.series+1} из 9 · заезд ${cur.beat+1} из 5`)+form;
