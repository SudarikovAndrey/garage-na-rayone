...
export function bindNotifications(){
 ...
 function refresh(){
  ...
  // Входящие стрелки: число кладёт game.js после запроса списка (data-incoming), сами сервер не опрашиваем.
  const strelka=document.querySelector('#challenge-button'),incoming=Number(strelka?.dataset.incoming||0);if(strelka&&incoming>0)wanted.set(strelka,String(incoming));
  ...
  // Чистим и осиротевшие метки: уборка выше ищет хозяев по классу notice-host, а класс теряется,
  // если узлу переписали className или его перенесли в другой контейнер. Метка тогда оставалась
  // висеть — и рядом с настоящей появлялась вторая точка (её и ловил Андрей у кнопки машин).
  for(const m of document.querySelectorAll('.notice-mark'))if(!wanted.has(m.parentElement))m.remove();
