...
export function addEveningGarage(scene,room,atlas,level=inferLevel(room)){
 ...
 // Кусок атласа. Клон texture в three — это отдельная загрузка на видеокарту: четыре куска занимали
 // 32 МБ видеопамяти вместо восьми, хотя картинка одна. Поэтому смещение и масштаб куска запекаем в UV
 // геометрии (она тут всё равно сливается по материалам), а текстура у всех одна и та же.
 const TILE=(x,y)=>[.5,y?580/1254:674/1254,x*.5,y?0:580/1254];
 const tile=(m,x,y)=>{m.map=atlas;m.userData.atlasTile=TILE(x,y);return m;};
 // Асфальт — исключение: у бетонного пола UV выходят далеко за 0..1 и опираются на обрезку по краю
 // атласа, запекание сдвинуло бы рисунок. Ему остаётся собственный клон.
 const tileClone=(x,y)=>{const t=atlas.clone(),[rx,ry,ox,oy]=TILE(x,y);t.repeat.set(rx,ry);t.offset.set(ox,oy);t.needsUpdate=true;textures.push(t);return t;};
 const fabric=mat(0xb49b83),posterA=mat(0xffffff),posterB=mat(0xffffff),asphalt=mat(0x666974,.94);
 tile(fabric,0,1);tile(posterA,0,0);tile(posterB,1,0);asphalt.map=tileClone(1,1);
 ...
 function add(geo,m,pos,rot=[0,0,0],parent=owner){const transform=new T.Matrix4().compose(new T.Vector3(...pos),new T.Quaternion().setFromEuler(new T.Euler(...rot)),new T.Vector3(1,1,1));geo.applyMatrix4(transform);
  const t=m.userData.atlasTile,uv=t&&geo.attributes.uv;/* кусок атласа — в UV: см. tile() */
  if(uv){for(let i=0;i<uv.count;i++)uv.setXY(i,uv.getX(i)*t[0]+t[2],uv.getY(i)*t[1]+t[3]);uv.needsUpdate=true;}const key=parent.uuid+':'+m.uuid;if(!buckets.has(key))buckets.set(key,{parent,m,geos:[]});buckets.get(key).geos.push(geo.index?geo.toNonIndexed():geo);if(geo.index)geo.dispose();}
 ...
 const sofaSpot=[-3.57,0,.68],sofa=cluster('Sofa',sofaSpot,0,.9);sofa.userData.footprint=[1.02*.9,2.25*.9];
 ...
 const tvSpot=[-1.60,0,-3.22],tvYaw=Math.atan2(sofaSpot[0]-tvSpot[0],sofaSpot[2]-tvSpot[2]),tv=cluster('TV',tvSpot,tvYaw,.75);
 box([0,.36,0],[.94,.09,.62],wood,.012,tv);for(const x of [-.39,.39])for(const z of [-.24,.24])box([x,.17,z],[.07,.34,.07],wood,.008,tv);
 box([0,.11,0],[.80,.03,.50],wood,.008,tv);for(const x of [-.3,.3])box([x,.4125,0],[.08,.015,.3],black,.002,tv);
 ...
 for(let i=0;i<4;i++){box([-.31+i*.19,.1525,.02],[.16,.055,.27],black,.006,tv);box([-.31+i*.19,.182,.04],[.11,.004,.15],paper,0,tv);}
