...
export function asphaltDetail(wet=0){
 ...
 for(let y=0;y<n;y++)for(let x=0;x<n;x++){
  ...
  const tilt=(lattice((x+.5)/n,y/n,5,1)-lattice((x-.5)/n,y/n,5,1))*40;// пологие наплывы: наклон нормали по крупным пятнам
  norm[i]=128+(f-.5)*(wet?15:32)+tilt;norm[i+1]=128+Math.sin(grain)*(wet?7:16);norm[i+2]=254;norm[i+3]=255;
  // Крупные пятна и на сухом: асфальт едет, и без них на ровном участке глазу не за что зацепиться.
  const r=wet?90+f*50+blot*18:190+f*60+blot*22;
