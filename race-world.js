...
// а не обрывается.
 scene.fog=new T.FogExp2(cfg.fog,cfg.night?.016:cfg.wet?.0105:.0065);scene.environment=environment;scene.environmentIntensity=cfg.night?1.45:cfg.wet?.85:.75;/* ночью снимок окружения — огни города и луна, лаку нужны блики */
...
// Ночью под каждым фонарём на земле лежит пятно света. Настоящий источник есть только у редких холодных ламп,
 ...
 const poolMaterial=(color,strength)=>{const m=new T.ShaderMaterial({transparent:true,depthWrite:false,fog:false,blending:T.AdditiveBlending,
  uniforms:{tint:{value:new T.Color(color).multiplyScalar(strength)}},
  vertexShader:'varying vec2 vUv;void main(){vUv=uv;gl_Position=projectionMatrix*modelViewMatrix*vec4(position,1.);}',
  fragmentShader:'precision highp float;varying vec2 vUv;uniform vec3 tint;void main(){float r=min(1.,length(vUv-.5)*2.);float f=pow(1.-r,2.2);gl_FragColor=vec4(tint*f,0.);}'});
  m.name='LampPool';mats.set('pool'+m.id,m);return m;};
 const pools=cfg.night?{warm:poolMaterial(0xffa84e,.42),cold:poolMaterial(0xbfe0ff,.30)}:null;
 function lampPool(g,x,z,cold){const o=new T.Mesh(new T.PlaneGeometry(1,1),cold?pools.cold:pools.warm);o.rotation.x=-Math.PI/2;o.position.set(x,.032,z);const r=cold?5.2:4.6;o.scale.set(r*2,r*2,1);o.castShadow=o.receiveShadow=false;g.add(o);return o;}
 ...
 function lamp(g,x,z,lit=false){cyl(g,.065,6,[x,3,z],steel);beam(g,[x,5.9,z],[x-Math.sign(x)*1.4,6.15,z]);const cold=cfg.night&&lit;box(g,[.65,.10,.35],[x-Math.sign(x)*1.35,6.12,z],mat(cold?0xe4f1ff:0xf0d9a0,.45,.1,cfg.night?(cold?0xbfe0ff:0xffc273):0));
  if(cfg.night)lampPool(g,x-Math.sign(x)*1.35,z,cold);
 ...
 let scrolled=0;const roadLength=470;
 function update(travel=0){const delta=travel-scrolled;if(!delta)return;scrolled=travel;for(const map of [roadMat.map,roadMat.normalMap,roadMat.roughnessMap]){if(!map)continue;map.offset.y=(map.offset.y+delta*map.repeat.y/roadLength)%1;}}
 ...
 return {scene,moving,cfg,update,dispose};
}
