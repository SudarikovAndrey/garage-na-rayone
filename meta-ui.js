...
export function bindMeta({save,openCustomizer,changed,refreshCar,startRace,openDuels,selectCar,renderFleetPreviews,toast,sfx,profiles=null,identity=()=>({})}){
 ...
 else if(mode==='profile'){const me=identity()||{};$('#meta-body').innerHTML=profiles?profileMarkup(profiles,save,cloudList,{required:requireProfile,dev:devMode(),photo:me.photo||null,name:me.name||null}):'<p class="crate-note">Профили недоступны.</p>';}
 ...
 $('#meta-body').addEventListener?.('submit',e=>{const f=e.target.closest('form[data-profile-create]');if(!f||!profiles)return;e.preventDefault();const name=cleanName(new FormData(f).get('name'));if(!name)return;const current=profiles.current();if(!current){const btn=f.querySelector?.('button[type=submit]');if(btn)btn.disabled=true;profiles.adoptOrContinue(name,save).then(r=>{if(btn)btn.disabled=false;if(!r)return;track('profile',{action:'named',continued:r.continued});if(r.continued)return;changed();if(requireProfile){nameAccepted();toast('Погнали, '+name);closeDialog(dialog);}else{toast('Теперь ты '+name);render();}});}else{profiles.create(name,save);profiles.select(name,save);}});
