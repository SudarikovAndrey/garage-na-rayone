...
export const MOLDING={samara:[.55,.60],volga:[.47,.53]};
export function anchors(fit=DEFAULT_FIT){
 ...
 // Двери — по швам модели (body.doors: весь дверной пояс, body.plateDoor: дверь водителя, метры по длине, нос в −X).
 // Без них — промежуток между арками. Номер встаёт на середину двери водителя (Андрей, 23 сентября: номер
 // стоял посередине между арками — у седанов это стойка между дверями, у Восьмёрки — задний край двери).
 const span=(x0,x1)=>{const r={u0:Math.max(.06,(x0+L/2)/L),u1:Math.min(.94,(x1+L/2)/L)};r.c=(r.u0+r.u1)/2;r.w=r.u1-r.u0;return r;};
 const door=b.doors?span(...b.doors):span(ax-wb/2+r+.15+.02*L,ax+wb/2-r-.15-.02*L),plate=b.plateDoor?span(...b.plateDoor):door;
 // Ручка двери: у задней кромки двери водителя, чуть ниже пояса. Текст туда не ставим.
 const handle={u0:plate.u1-.11,u1:plate.u1-.005,v0:belt-.23,v1:belt-.10};
 ...
 return {L,H,W,belt,beltM,floor,hoodCut,uF,uR,ru,archTop,door,plate,handle,mold,hoodEnd,wing:{u0:.05,u1:uF+ru},quarter:{u0:uR-ru,u1:Math.min(.96,uR+ru+.04)},roof:{u0:hoodEnd+.08,u1:.9}};
}
...
export function layoutLivery(spec,fit=DEFAULT_FIT,paint='#9d2730'){
 ...
 for(const g of spec.graphics||[]){
  ...
  switch(g.kind){
   ...
   case 'split':{
    ...
    break;}
   case 'step':{/* высокий порог (Нива): ступенька отсчитывается как у легковой, иначе краска ползёт выше двери */const v1=Math.min(A.belt-.03,A.floor+g.h-Math.max(0,A.floor-.20)*1.4);side({kind:'poly',points:[[g.at,band?band.v1:A.floor],[g.at+.05,v1],[1,v1],[1,0],[g.at,0]],fill});remember(g.at+.05,0,1,v1,fill);break;}
   case 'swoosh':{
 ...
 if(spec.number){
  ...
  const uAt=n.at==='quarter'?Math.min(A.quarter.u1-.02-R*A.H/A.L,Math.max(A.quarter.u0+R*A.H/A.L*.5,A.uR)):(spec.sponsors?.title?A.plate.u0+A.plate.w*.30:A.plate.c);
