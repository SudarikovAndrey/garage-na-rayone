...
import {grantCrate,crateById} from './crates.js';
...
function applyGrant(save,grant,reason){
 ...
 // Ящик приходит и списком, и одиночным полем: {crate:'street'} выписывают стрелки и лестница
 // корешей. Раньше одиночное поле терялось молча — квитанция считалась применённой, а ящика не было.
 const crates=[...(Array.isArray(grant.crates)?grant.crates:[]),...(typeof grant.crate==='string'?[grant.crate]:[])];
 for(const id of crates)if(crateById(id)){grantCrate(save,id,reason);got.crates.push({id});}
