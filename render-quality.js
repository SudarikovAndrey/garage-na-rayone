...
export class AdaptiveQuality{
 ...
 sample(seconds){if(!Number.isFinite(seconds)||seconds<=0||seconds>.25)return false;if(this.cooldown>0){this.cooldown-=seconds;return false;}this.elapsed+=seconds;this.frames++;if(seconds>.039)this.slow++;if(seconds<.019)this.fast++;if(this.elapsed<4)return false;const bad=this.slow/this.frames>.35,good=this.fast/this.frames>.96;this.goodSeconds=good?(this.goodSeconds||0)+this.elapsed:0;let next=this.level;if(bad)next=Math.max(0,next-1);else if(this.goodSeconds>24)next=Math.min(2,next+1);this.elapsed=this.slow=this.fast=this.frames=0;if(next===this.level)return false;this.level=next;this.goodSeconds=0;this.cooldown=4;return true;}
