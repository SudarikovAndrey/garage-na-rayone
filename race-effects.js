...
// Кузов на подвеске. Раньше это были два синуса — 5.4 Гц по высоте и 2.9 Гц по крену, привязанные к скорости:
// машина мелко дрожала на ходу, как будто ничего не весит и стоит без амортизаторов (Андрей, 17 сентября).
// Настоящий кузов раскачивается около 1.2 Гц и только когда его качнули: разгоном, торможением, переключением.
// Поэтому здесь пружина с демпфером, а не колебание по часам.
//
// Ускорение приходит как разность скоростей за кадр — величина шумная, и если гнать её прямо в наклон, кузов
// дёргается покадрово. Сначала фильтр, потом пружина: получается запаздывание и мягкий доворот, как у тяжёлой машины.
export const BODY={
 tau:.16,        // сглаживание ускорения, с
 pitchPerG:.0038,// приседание на корму за 1 м/с² продольного ускорения
 squat:.0010,    // и просадка кузова на ту же нагрузку, м: сантиметр на полном разгоне, не больше —
                 // иначе машина на глаз проваливается в дорогу
 omega:7.4,      // собственная частота кузова, рад/с (1.18 Гц)
 zeta:.45,       // затухание: один мягкий доворот, без дрожи и без раскачки
 kick:.085,      // доворот от толчка переключения. Не импульс в скорость: игра шлёт толчок каждый кадр,
                 // пока он затухает, и импульсы складывались бы в раскачку. Это добавка к цели, за которой
                 // пружина идёт и сама же возвращается, когда толчок сошёл на нет.
};
export class CarBody{
 constructor(){this.reset();}
 reset(){this.pitch=0;this.vp=0;this.height=0;this.vh=0;this.accel=0;}
 update(dt,{speed=0,acceleration=0,kick=0,reduced=false}={}){
  if(reduced){this.reset();return {height:0,pitch:0,roll:0};}
  const step=Math.min(Math.max(dt,0),.05);
  this.accel+=(acceleration-this.accel)*(1-Math.exp(-step/BODY.tau));
  const load=T.MathUtils.clamp(this.accel,-9,9),rolling=Math.min(1,speed/6);
  const targetPitch=-load*BODY.pitchPerG*rolling+Math.max(0,kick)*BODY.kick;
  const targetHeight=-Math.abs(load)*BODY.squat*rolling;
  const k=BODY.omega*BODY.omega,c=2*BODY.zeta*BODY.omega;
  for(let left=step;left>0;){
   const h=Math.min(left,1/120);left-=h;
   this.vp+=((targetPitch-this.pitch)*k-this.vp*c)*h;this.pitch+=this.vp*h;
   this.vh+=((targetHeight-this.height)*k-this.vh*c)*h;this.height+=this.vh*h;
  }
  return {height:this.height,pitch:this.pitch,roll:0};
 }
}
