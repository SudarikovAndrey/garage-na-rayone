...
// Кузов на заплатках и колее. Пружина была почти критически задемпфированной и жёсткой (2.2 Гц, ζ≈.87):
// кузов повторял каждую неровность как приклеенный, и это читалось как машина без амортизаторов. У тяжёлой
// машины на мягких пружинах частота около 1.4 Гц и затухание ζ≈.42 — она съедает мелочь и отвечает одним
// пологим доворотом (Андрей, 17 сентября).
const RIDE={omega:8.6,zeta:.42};
...
export class RoadRide{
 ...
 const values=[];for(const axle of [-wb/2+offset,wb/2+offset])for(const side of [-1,1]){const x=model.position.x+side*w,z=model.position.z+axle;/* Медленно катящаяся машина заплатки почти не чувствует: подвеска их съедает. Подскоки — только на скорости. */const pace=Math.min(1,speed/25);let h=this.sample(x,z,z+Math.max(0,delta))*(.4+.6*pace);if(soft)h+=.075*soft*roughBump(travel-axle+side*.9)*(.35+.65*pace);values.push(h);}
 ...
 for(let remaining=dt;remaining>0;){const step=Math.min(remaining,1/120);remaining-=step;for(const [key,v] of [['height','vh'],['pitch','vp'],['roll','vr']]){s[v]+=((targets[key]-s[key])*RIDE.omega*RIDE.omega-s[v]*2*RIDE.zeta*RIDE.omega)*step;s[key]+=s[v]*step;}}
 return s;
 }
}
