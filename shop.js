...
export const PART_OFFERS=PARTS.filter(p=>p.rarity>0&&p.rarity<4&&!p.style&&!p.variant)/* магазин продаёт только базовые варианты обычных редкостей: стили, версии и уникальные — добыча */.map(p=>({id:p.id,currency:p.rarity===3?'hard':'cash',cashPrice:p.rarity===3?160000:null,price:p.rarity===3?420:p.rarity===2?26000:9000}));
