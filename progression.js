...
import {CARS,CAR_COUNT,FLEET_REVISION} from './fleet.js';
import {migrateFleet} from './fleet-migration.js';
...
export const CAR_SHARD_COSTS=[0,50,90,100,16];// двенашка и нива — по ценам прежних Десятки и Нивы
...
export const SLOW_CARS=new Set();// медленные внедорожники ушли из парка 22 сентября
// 22 сентября в парке три машины: все боссы на Волге. Копейка на 804 м не держит темп Волги, которая с
// третьего района — машина самого игрока, и босс переставал быть стеной (check-chase, ранг 134).
export const BOSS_CARS=[2,2,2,2,2];
...
export const STAGE_CARS=[[1],[1,0],[0,1,2],[0,2,1],[2,0]];// номера нового парка (три машины)
...
export function hydrate(d={}){d=migrateFleet(d)||{};const oldRank=integer(d.rank??Math.min(d.wins||0,24),0,d.campaignRevision===CAMPAIGN_REVISION?CAMPAIGN_LENGTH:25);const selected=integer(d.selected,0,CAR_COUNT-1),hasCollection=Number(d.collectionRevision)>=1;const s={tutorials:Object.fromEntries(['drag-auto','drag-manual','drift'].filter(id=>d.tutorials?.[id]===true).map(id=>[id,true])),hints:Array.isArray(d.hints)?d.hints.filter(x=>typeof x==='string'):[],campaignRevision:CAMPAIGN_REVISION,collectionRevision:1,fleetRevision:FLEET_REVISION,updatedAt:integer(d.updatedAt,0,4e12),campaignChampionships:integer(d.campaignChampionships,0,10000),campaignLosses:integer(d.campaignLosses,0,999),campaignSeconds:integer(d.campaignSeconds,0,1e8),campaignAttempts:integer(d.campaignAttempts,0,1e6),paint:CARS.map((car,i)=>typeof d.paint?.[i]==='string'?d.paint[i]:car.color)/* право на краску сверяет restorePaints: жёсткий список id сбрасывал купленные цвета при каждом перезаходе */,version:3,cash:integer(d.cash??1200,0,1e8),selected,garage:restoreGarageLevel(d),garageRevision:2,records:CARS.map((car,i)=>Number(d.records?.[i])>0?Number(d.records[i]):null),wins:integer(d.wins,0,1e6),rank:d.campaignRevision===CAMPAIGN_REVISION?oldRank:oldRank*9,races:integer(d.races,0,1e6),streak:integer(d.streak,0,1e6),scrap:integer(d.scrap,0,1e7),boxes:integer(d.boxes??1,0,10000),bossBoxes:integer(d.bossBoxes,0,10000),pity:integer(d.pity,0,7),inventory:{},equipped:CARS.map(()=>({})),carShards:CARS.map((_,i)=>integer(d.carShards?.[i],0,1e6)),unlockedCars:CARS.map((_,i)=>hasCollection?!!d.unlockedCars?.[i]:i===0||i===selected)};
